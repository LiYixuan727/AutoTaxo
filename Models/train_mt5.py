...
from transformers import AdamW, MT5ForConditionalGeneration, AutoTokenizer, get_linear_schedule_with_warmup
...
class HyperGenerationDataset(Dataset):
	def __init__(self, tokenizer, data_dir, type_path, parse_version, max_len=None):
		self.path = os.path.join(data_dir, type_path + '_hH_{}.csv'.format(parse_version))
...
def get_dataset(tokenizer, type_path, args):
	return HyperGenerationDataset(tokenizer=tokenizer, data_dir=args.data_dir, type_path=type_path, parse_version=args.parse_version,  max_len=args.max_seq_length)
...
if __name__ == '__main__':
	...
	VER = "ipc8_simple"
	#VER = "ipc6" 
	PARSE_VER = 'EN_{}'.format(VER)

	args_dict.update({'data_dir': 'data/{}'.format(VER), 'output_dir': 'output/{}'.format(VER), 'num_train_epochs':3, 'max_seq_length':32, 'parse_version':PARSE_VER})
	...
	#checkpoint_callback = pl.callbacks.ModelCheckpoint(period =1,filepath=args.output_dir,  monitor="val_loss", mode="min", save_top_k=1, prefix="checkpoint")
	...
	print ("Saving model")
	model.model.save_pretrained("result/"+PARSE_VER)
	print ("Saved model")
